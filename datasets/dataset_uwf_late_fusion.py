...
def parse_lesion_vector(lesion_value):
    lesion_list = ast.literal_eval(str(lesion_value))

    if len(lesion_list) != 7:
        raise ValueError(f"Expected lesion vector of length 7, got {len(lesion_list)}: {lesion_value}")

    return torch.tensor(lesion_list, dtype=torch.float32)
...
class UWFLateFusionDataset(Dataset):
    def __init__(self, csv_path, images_dir, transform=None, use_clahe=True):
        ...
        self.lesion_col = "lesion"

        required_cols = [self.image_col, self.label_col, self.lesion_col]
    ...
    def __getitem__(self, idx):
        ...
        lesion_vector = parse_lesion_vector(row[self.lesion_col])
        ...
        return image, lesion_vector, label, quality_weight, quality_raw, image_name